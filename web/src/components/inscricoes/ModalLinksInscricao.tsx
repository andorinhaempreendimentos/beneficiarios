                          ...
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${isCopied ? "bg-emerald-600 text-white shadow-xs" : "bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-300"}`}
                              >
                                {isCopied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                                <span>{isCopied ? "Copiado!" : "Copiar Link"}</span>
                              </button>
                              <a href={url} target="_blank" rel="noreferrer" className="p-1.5 text-zinc-400 hover:text-amber-600 hover:bg-zinc-100 rounded-lg transition-colors" title="Abrir página em nova guia">
                                <ExternalLink className="h-4 w-4" />
                              </a>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-wrap text-[11px]">
                            {org?.nome && <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-800 px-2.5 py-1 rounded-md font-medium border border-emerald-200">🏛️ {org.nome}</span>}
                            {obj?.nome && <span className="inline-flex items-center gap-1 bg-purple-50 text-purple-800 px-2.5 py-1 rounded-md font-medium border border-purple-200">📄 {obj.nome}</span>}
                          </div>
