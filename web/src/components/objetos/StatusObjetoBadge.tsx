...
import { statusObjetoTone, statusObjetoLabel } from "@/lib/status";
import type { StatusObjeto } from "@/lib/types";
import { useToast } from "@/components/providers/ToastProvider";

const OPCOES: { value: StatusObjeto; label: string; tone: keyof typeof statusObjetoTone }[] = [
  { value: "ativo", label: "Ativo", tone: "green" },
  { value: "planejado", label: "Planejado", tone: "sky" },
  { value: "encerrado", label: "Encerrado", tone: "zinc" },
];

interface StatusObjetoBadgeProps {
  objetoId: string;
  statusAtual?: string | null;
  onStatusChange?: (novoStatus: StatusObjeto) => void;
}

export function StatusObjetoBadge({
  objetoId,
  statusAtual,
  onStatusChange,
}: StatusObjetoBadgeProps) {
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [statusState, setStatusState] = useState<StatusObjeto>(
    (statusAtual as StatusObjeto) || "ativo"
  );

  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (statusAtual) setStatusState(statusAtual as StatusObjeto);
  }, [statusAtual]);
  ...
  async function handleSelect(novoStatus: StatusObjeto) {
    setOpen(false);
    ...
    try {
      await objetosApi.update(objetoId, { status: novoStatus });
      toast.success(`Status do objeto alterado para ${statusObjetoLabel[novoStatus]}!`);
      if (onStatusChange) onStatusChange(novoStatus);
    } catch (err: any) {
      setStatusState(statusAntigo);
      toast.error(err?.message || "Erro ao alterar status do objeto.");
    } finally {
      setLoading(false);
    }
  }

  const tone = statusObjetoTone[statusState] ?? "zinc";
  const label = statusObjetoLabel[statusState] ?? statusState;
