-- Migration 049: Corrigir consultar_inscricao_publica para inscrições sem turma
-- Troca JOIN turmas por LEFT JOIN, e busca núcleo via i.nucleo_id quando turma_id é NULL.
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'nucleo_endereco', COALESCE(n.endereco, '') ||
      CASE WHEN n.numero IS NOT NULL AND n.numero <> '' THEN ', ' || n.numero ELSE '' END ||
      CASE WHEN n.bairro IS NOT NULL AND n.bairro <> '' THEN ' - ' || n.bairro ELSE '' END
  ) INTO v_res
  FROM inscricoes i
  ...
  LEFT JOIN nucleos n ON n.id = COALESCE(t.nucleo_id, i.nucleo_id)
  WHERE i.id = p_id;
